		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h2 class="modal-title">Excluir tipo de imagem</h2>
				...
				<div class="modal-body">
					<div class="user-profile">
						<h3 class="profile-name"><i class="fas fa-image" aria-hidden="true"></i> {{ imageType.name }}</h3>
					</div>
					Deseja realmente excluir o tipo de imagem?

				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" @click="closeModal">Voltar</button>
					<button type="button" class="btn btn-danger" @click="deleteImageType">Excluir</button>
				</div>
			</div>
		</div>
	...
	methods: {
		...
		deleteImageType() {
			...
			if (token != undefined) {
				...
				axios.delete(`http://localhost:8081/api/image-type/${this.imageType._id}`, req)
					.then(response => {
