			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="user-profile">
						<h3 class="profile-name"><i class="fa fa-database" aria-hidden="true"></i> {{ solicitation.data.name
						}}</h3>
					</div>

					<label for="examType" class="form-label mt-3">Tipo de exame</label>
					<input class="form-control" type="text" :value=examType aria-label="Disabled examType" disabled
						readonly>

					<label for="description" class="form-label mt-3">Descrição</label>
					<textarea class="form-control" id="description" rows="3" disabled readonly
						v-model="this.description"></textarea>

					<label for="sourceLink" class="form-label mt-3">Link de origem</label>
					<input class="form-control mb-3" type="text" :value=sourceLink aria-label="Disabled sourceLink" disabled
						readonly>
				</div>
			</div>
		</div>
