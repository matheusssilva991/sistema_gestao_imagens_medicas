<template>
    <div class="container ms-3 me-3">
      <div class="sidebar">
        <div class="sidebar-header">
          <h2 class="sidebar-title">Tipos de imagem
            <button v-if="user.role" class="add-button" @click="openModalCreateImageTypeModalCompVue()">+ Novo</button>
          </h2>
          <div id="filter">
            <div class="filter-item-container">
              <i class="fa fa-search mt-2" aria-hidden="true"></i>
              <InputComp class="filter-item" name="pesquisa" type="pesquisar" placeHolder="Pesquisar tipo de imagem" :function="changeValues"></InputComp>
            </div>
          </div>
        </div>
        ...
        <nav class="pagination justify-content-center">
          <ul class="pagination">
            <li class="page-item">
              <button @click="paginaAnterior" :disabled="paginaAtual === 1" class="page-link btn-anterior">Anterior</button>
            </li>
            <li class="page-item">
              <button @click="proximaPagina" :disabled="!existemMaisPaginas" class="page-link btn-proximo">Próxima</button>
            </li>
          </ul>
        </nav>
      </div>
      ...
      <ShowImageTypeModalCompVue v-if="showViewImageTypeModalCompVue" :imageType="selectedImageType" @close-modal="closeModal" />
